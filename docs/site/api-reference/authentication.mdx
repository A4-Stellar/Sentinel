...
description: "How API key authentication works in Sentinel — the X-API-Key header, 401 vs 403 responses, and managing keys in self-hosted deployments."
---

Every Sentinel API endpoint (except `GET /v1/health`) requires authentication via an API key.
...
```bash
curl https://api.sentinel.a4stellar.com/v1/events \
...
Sign up at [sentinel.a4stellar.com](https://sentinel.a4stellar.com/signup). Free tier keys are issued immediately. Each key is tied to a rate-limit tier (see [Rate Limiting](/api-reference/rate-limiting)).
