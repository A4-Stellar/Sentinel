...
import (
	...
	apigrpc "github.com/Depo-dev/sentinel/services/api/grpc"
...
// note the sentinel_api_indexer_* prefix, which avoids colliding with the
...
// sentinel:events Redis Stream length. Mount at GET /metrics.
func MetricsHandler(pool *pgxpool.Pool, rdb *redis.Client) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		w.Header().Set("Content-Type", "text/plain; version=0.0.4; charset=utf-8")
		_, _ = fmt.Fprintf(w, "# HELP sentinel_indexer_lag_ledgers Number of ledgers the indexer is behind the chain tip.\n")
		_, _ = fmt.Fprintf(w, "# TYPE sentinel_indexer_lag_ledgers gauge\n")
		_, _ = fmt.Fprintf(w, "sentinel_indexer_lag_ledgers %g\n", metricLagLedgers.Get())
		_, _ = fmt.Fprintf(w, "# HELP sentinel_indexer_lag_seconds_estimated Estimated wall-clock lag: lag_ledgers * average ledger close time (issue #294).\n")
		_, _ = fmt.Fprintf(w, "# TYPE sentinel_indexer_lag_seconds_estimated gauge\n")
		_, _ = fmt.Fprintf(w, "sentinel_indexer_lag_seconds_estimated %g\n", metricLagSecondsEstimated.Get())
		_, _ = fmt.Fprintf(w, "# HELP sentinel_indexer_last_poll_timestamp_seconds Unix timestamp of the last successful indexer poll.\n")
		_, _ = fmt.Fprintf(w, "# TYPE sentinel_indexer_last_poll_timestamp_seconds gauge\n")
		_, _ = fmt.Fprintf(w, "sentinel_indexer_last_poll_timestamp_seconds %g\n", metricLastPollTimestamp.Get())
		_, _ = fmt.Fprintf(w, "# HELP sentinel_indexer_events_total Cumulative events indexed.\n")
		_, _ = fmt.Fprintf(w, "# TYPE sentinel_indexer_events_total gauge\n")
		_, _ = fmt.Fprintf(w, "sentinel_indexer_events_total %g\n", metricEventsTotal.Get())
		apigrpc.WriteClientMetrics(w)

		// Webhook delivery metrics (#241).
		_, _ = fmt.Fprintf(w, "# HELP sentinel_webhook_deliveries_success_total Successful webhook deliveries since startup.\n")
		_, _ = fmt.Fprintf(w, "# TYPE sentinel_webhook_deliveries_success_total counter\n")
		_, _ = fmt.Fprintf(w, "sentinel_webhook_deliveries_success_total %d\n", metricWebhookSuccess.Load())
		_, _ = fmt.Fprintf(w, "# HELP sentinel_webhook_deliveries_failed_total Failed (retryable) webhook deliveries since startup.\n")
		_, _ = fmt.Fprintf(w, "# TYPE sentinel_webhook_deliveries_failed_total counter\n")
		_, _ = fmt.Fprintf(w, "sentinel_webhook_deliveries_failed_total %d\n", metricWebhookFailed.Load())
		_, _ = fmt.Fprintf(w, "# HELP sentinel_webhook_deliveries_dead_lettered_total Webhook deliveries exhausted all retries.\n")
		_, _ = fmt.Fprintf(w, "# TYPE sentinel_webhook_deliveries_dead_lettered_total counter\n")
		_, _ = fmt.Fprintf(w, "sentinel_webhook_deliveries_dead_lettered_total %d\n", metricWebhookDeadLetter.Load())
		total := metricWebhookTotal.Load()
		var meanMs float64
		if total > 0 {
			meanMs = float64(metricWebhookDurationMs.Load()) / float64(total)
		}
		_, _ = fmt.Fprintf(w, "# HELP sentinel_webhook_delivery_mean_duration_ms Mean delivery round-trip latency in milliseconds.\n")
		_, _ = fmt.Fprintf(w, "# TYPE sentinel_webhook_delivery_mean_duration_ms gauge\n")
		_, _ = fmt.Fprintf(w, "sentinel_webhook_delivery_mean_duration_ms %g\n", meanMs)

		_, _ = fmt.Fprint(w, "# HELP sentinel_api_indexer_lag_ledgers Number of ledgers the indexer is behind the chain tip, as last observed via GET /v1/stats/indexer.\n")
		_, _ = fmt.Fprint(w, "# TYPE sentinel_api_indexer_lag_ledgers gauge\n")
		_, _ = fmt.Fprintf(w, "sentinel_api_indexer_lag_ledgers %g\n", metricLagLedgers.Get())
		_, _ = fmt.Fprint(w, "# HELP sentinel_api_indexer_last_poll_timestamp_seconds Unix timestamp of the last successful indexer poll, as last observed via GET /v1/stats/indexer.\n")
		_, _ = fmt.Fprint(w, "# TYPE sentinel_api_indexer_last_poll_timestamp_seconds gauge\n")
		_, _ = fmt.Fprintf(w, "sentinel_api_indexer_last_poll_timestamp_seconds %g\n", metricLastPollTimestamp.Get())
		_, _ = fmt.Fprint(w, "# HELP sentinel_api_indexer_events_indexed Cumulative events indexed, as last observed via GET /v1/stats/indexer.\n")
		_, _ = fmt.Fprint(w, "# TYPE sentinel_api_indexer_events_indexed gauge\n")
		_, _ = fmt.Fprintf(w, "sentinel_api_indexer_events_indexed %g\n", metricEventsTotal.Get())
		...
		if pool != nil {
			stat := pool.Stat()
			writeGauge(w, "sentinel_api_db_pool_acquired_connections", "Connections currently checked out of the API's Postgres pool.", float64(stat.AcquiredConns()))
			writeGauge(w, "sentinel_api_db_pool_idle_connections", "Idle (available) connections in the API's Postgres pool.", float64(stat.IdleConns()))
			writeGauge(w, "sentinel_api_db_pool_total_connections", "Total connections (idle + acquired) currently open in the API's Postgres pool.", float64(stat.TotalConns()))
			writeGauge(w, "sentinel_api_db_pool_max_connections", "Configured maximum size of the API's Postgres pool.", float64(stat.MaxConns()))
		}
		...
		if rdb != nil {
			...
			if length, err := rdb.XLen(ctx, eventStreamKey).Result(); err == nil {
				writeGauge(w, "sentinel_api_redis_stream_length", "Length of the sentinel:events Redis Stream (indexer -> API consumer backlog).", float64(length))
			}
		}

		// SSE + WS/GraphQL slow-consumer backpressure metrics (#224).
		_, _ = fmt.Fprintf(w, "# HELP sentinel_sse_slow_consumer_disconnects_total SSE connections closed because a write exceeded the write deadline.\n")
		_, _ = fmt.Fprintf(w, "# TYPE sentinel_sse_slow_consumer_disconnects_total counter\n")
		_, _ = fmt.Fprintf(w, "sentinel_sse_slow_consumer_disconnects_total %d\n", metricSSESlowConsumerDisconnects.Load())
		ws.WriteMetrics(w)
		...
		_, _ = fmt.Fprintf(w, "# HELP sentinel_ratelimit_key_allowed_total Requests allowed by the per-API-key tiered rate limiter.\n")
		_, _ = fmt.Fprintf(w, "# TYPE sentinel_ratelimit_key_allowed_total counter\n")
		_, _ = fmt.Fprintf(w, "sentinel_ratelimit_key_allowed_total %d\n", rlAllowedN)
		_, _ = fmt.Fprintf(w, "# HELP sentinel_ratelimit_key_rejected_total Requests rejected by the per-API-key tiered rate limiter.\n")
		_, _ = fmt.Fprintf(w, "# TYPE sentinel_ratelimit_key_rejected_total counter\n")
		_, _ = fmt.Fprintf(w, "sentinel_ratelimit_key_rejected_total %d\n", rlRejectedN)

		ipAllowedN, ipRejectedN, globalAllowedN, globalRejectedN := middleware.AbuseMetrics()
		_, _ = fmt.Fprintf(w, "# HELP sentinel_ratelimit_ip_allowed_total Requests allowed by the pre-auth per-IP rate limiter.\n")
		_, _ = fmt.Fprintf(w, "# TYPE sentinel_ratelimit_ip_allowed_total counter\n")
		_, _ = fmt.Fprintf(w, "sentinel_ratelimit_ip_allowed_total %d\n", ipAllowedN)
		_, _ = fmt.Fprintf(w, "# HELP sentinel_ratelimit_ip_rejected_total Requests rejected by the pre-auth per-IP rate limiter.\n")
		_, _ = fmt.Fprintf(w, "# TYPE sentinel_ratelimit_ip_rejected_total counter\n")
		_, _ = fmt.Fprintf(w, "sentinel_ratelimit_ip_rejected_total %d\n", ipRejectedN)
		_, _ = fmt.Fprintf(w, "# HELP sentinel_concurrency_allowed_total Requests allowed through the global concurrency cap.\n")
		_, _ = fmt.Fprintf(w, "# TYPE sentinel_concurrency_allowed_total counter\n")
		_, _ = fmt.Fprintf(w, "sentinel_concurrency_allowed_total %d\n", globalAllowedN)
		_, _ = fmt.Fprintf(w, "# HELP sentinel_concurrency_rejected_total Requests shed by the global concurrency cap.\n")
		_, _ = fmt.Fprintf(w, "# TYPE sentinel_concurrency_rejected_total counter\n")
		_, _ = fmt.Fprintf(w, "sentinel_concurrency_rejected_total %d\n", globalRejectedN)
		_, _ = fmt.Fprintf(w, "# HELP sentinel_concurrency_in_flight Requests currently in flight.\n")
		_, _ = fmt.Fprintf(w, "# TYPE sentinel_concurrency_in_flight gauge\n")
		_, _ = fmt.Fprintf(w, "sentinel_concurrency_in_flight %d\n", middleware.InFlightRequests())
	}
}
