...
ADMIN_IDENTITY="${ADMIN_IDENTITY:-sentinel-admin}"
...
for name in sentinel-holder-a sentinel-holder-b; do
  stellar keys generate --fund --network "$NETWORK" --overwrite "$name"
done
HOLDER_A=$(stellar keys address sentinel-holder-a)
HOLDER_B=$(stellar keys address sentinel-holder-b)
...
stellar contract invoke --id "$CONTRACT_ID" --source sentinel-holder-a --network "$NETWORK" \
  -- transfer --from "$HOLDER_A" --to "$HOLDER_B" --amount 250000

echo "3/5 approve holder B -> admin as spender"
stellar contract invoke --id "$CONTRACT_ID" --source sentinel-holder-b --network "$NETWORK" \
