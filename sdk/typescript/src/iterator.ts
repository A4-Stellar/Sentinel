 ...
 * shape of `SentinelClient.queryEvents`, so the client method can pass its own
...
export interface IterEventsOptions {
   ...
   * and the server still reports more results, a `SentinelError` with code
 ...
 * automatically until `has_more === false`. Any `SentinelError` thrown by the
 ...
 *
 * @throws {SentinelError} with code `ITERATION_LIMIT` if `maxPages` is exceeded
...
export async function* iterEvents(
  ...
  for (;;) {
    ...
    // A SentinelError thrown here is intentionally not caught — it propagates
