 ...
 */
export const ENV_API_KEY = "SENTINEL_API_KEY";
export const ENV_BASE_URL = "SENTINEL_BASE_URL";
...
export function resolveApiKey(apiKey: string | undefined): string {
  ...
  if (!resolved) {
    throw new SentinelError(
      "CONFIG",
      `Sentinel API key is required: pass apiKey in the client config or set the ${ENV_API_KEY} environment variable.`,
    );
  }
  return resolved;
}
...
export function resolveApiUrl(apiUrl: string | undefined): string {
  ...
  if (!resolved) {
    throw new SentinelError(
      "CONFIG",
      `Sentinel apiUrl is required: pass apiUrl in the client config or set the ${ENV_BASE_URL} environment variable.`,
    );
  }
  return resolved;
}
