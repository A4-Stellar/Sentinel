...
BACKUP_FILE="${BACKUP_DIR}/sentinel_prod_${TIMESTAMP}.sql.gz"
...
if pg_dump "${DATABASE_URL}" --format=custom --compress=9 | gzip > "${BACKUP_FILE}"; then
    ...
    find "${BACKUP_DIR}" -name "sentinel_prod_*.sql.gz" -mtime +30 -delete
    ...
    if [ -n "${ALERT_WEBHOOK_URL}" ]; then
        curl -s -X POST -H "Content-Type: application/json" \
            -d "{\"alert\": \"SentinelDatabaseBackupFailed\", \"severity\": \"critical\", \"message\": \"Scheduled database backup failed at ${TIMESTAMP}\"}" \
